...
const QuillEditorComponent = ({ body, handleQuillChange, mountBody }: any) => {
  const quillElement: any = useRef();
  const quillInstance: any = useRef();
  ...
  useEffect(() => {
    ...
    const typescriptWindow: any = window;
    if (quillElement.current && typescriptWindow.Quill) {
      ...
      quillInstance.current = new typescriptWindow.Quill(quillElement.current, {
        modules: {
          history: {
            delay: 2000,
            maxStack: 500,
  ...
  return (
    <div className="quillEditor-style" ref={quillElement}>
      hi
    </div>
  );
};
